...
class BinarySearchTree
{
 ...
 private:
	...
	//	vector<int> row;
      ...
      BinaryNode( const Comparable & theElement, BinaryNode *lt, BinaryNode *rt, int h = 0, int K = 0, bool d = false)
      : element{ theElement }, left{ lt }, right{ rt }, height{h},deleted{d} 
	{

	}

      BinaryNode( Comparable && theElement, BinaryNode *lt, BinaryNode *rt, int h = 0, int K = 0, bool d = false)
      : element{ std::move( theElement ) }, left{ lt }, right{ rt }, height{h}, deleted {d}
	{
	...
	else if( x < t->element || t->deleted != true)
	  {
	    Comparisons++;
	    insert( x, t->left);
	    //balance(t);
	  }
	else if( t->element < x || t->deleted != true)
	  {
	    Comparisons += 2;
	    insert( x, t->right);
	    //balance(t);
	...
	if( t == nullptr )
	  return nullptr;
	if( t->left == nullptr || t->deleted != true )
	  return t;
	...
	return findHeight( t->left, count+1 );
      ...
      {
	if( t != nullptr )
	  while( t->right != nullptr && t->deleted != 1 )
	    t = t->right;
	return t;
      }
	...
	if(t == nullptr || t->deleted == true)
	  return 0;
	else if(t!= nullptr || t->deleted != true)
	  {
	    countKeys(t->left);
	    countKeys(t->right);
	    Count_Keys +=2;
	...
	if( t != nullptr )
	  {
	    ...
	    if (t->deleted != true)
	      out << left << t->element<< "   "; //"\t"<< t->KeyComparison<<"\t"; // "\t\t\t";
